...
const Profile = ({ user }) => {
    ...
    const fetchUserDetails = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_SERVER_URI}/api/auth/user/${user.userId}`,
          {
            headers: {
  ...
  return (
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded shadow">
      ...
      <h2 className="text-2xl font-sans text-primary-100 font-bold">Your Profile</h2>
        <span className="capitalize text-sm text-accent-100 font-medium font-sans">Please update your profile for providing you the better accomdation.</span>
      </div>
